...
function Chinese({ theme, onToggleTheme }) {
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-6 rounded-2xl bg-white p-6 shadow-md dark:bg-gray-800 dark:shadow-slate-950/40 sm:p-8">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Enter a topic to generate clear composition guidance.
          </p>
          ...
          {result ? (
            <div className="mt-8 space-y-6 rounded-xl border border-slate-200 bg-slate-50 p-5 dark:border-gray-700 dark:bg-gray-900/50 sm:p-6">
              <section>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">开头</h2>
                <p className="mt-2 text-sm leading-7 text-slate-700 dark:text-slate-200">{result.开头 || '-'}</p>
              </section>

              <section>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">内容</h2>
                <ul className="mt-2 list-disc space-y-2 pl-5 text-sm leading-7 text-slate-700 dark:text-slate-200">
                  {result.内容.length ? result.内容.map((item, index) => <li key={index}>{item}</li>) : <li>-</li>}
                </ul>
              </section>

              <section>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">结尾</h2>
                <p className="mt-2 text-sm leading-7 text-slate-700 dark:text-slate-200">{result.结尾 || '-'}</p>
              </section>

              <section>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">好词好句</h2>
                <ul className="mt-2 flex flex-wrap gap-2">
                  {result.好词好句.length ? (
                    result.好词好句.map((phrase, index) => (
                      <li
                        key={index}
